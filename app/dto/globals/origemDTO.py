...
def validar_dados_origem(nome: str, id: int) -> Union[bool, str]:
    """Valida se o par de ID e Nome da origem existe e coincide no cache da aplicação.

    Args:
        nome (str): Nome da opção de origem informada.
        id (int): ID identificador da opção de origem no Podio.
    ...
    try:
        ...
        for item in cache_metadados:
            # Filtra apenas o bloco de chaves mapeado para a seletora de origem (como conheceu a AIESEC)
            if item.get("external_id") == "tag-origem-2":
                tem_origem = True  # Encontrou o bloco de origem no cache
                ...
                for opcao in item.get("options", []):
                    ...
                    if opcao.get("id") == id:
                        tem_id_origem = True  # O ID enviado existe nas opções ativas
        ...
        if not tem_origem:
            return "Não possui o external_id tag-origem-2 no cache"

        if not tem_id_origem:
            return "ID da origem informado não foi encontrado nas opções"
        else:
            return "O nome da origem não corresponde ao ID informado"
...
class Origem(BaseModel):
    """Modelo Pydantic que representa a origem (como conheceu a AIESEC) e valida dados via cache do Podio."""
    ...
    id: PositiveInt = Field(
        ...,
        description="ID de como conheceu a AIESEC",
        json_schema_extra={"example": 77},
    )
    nome: str = Field(
        ...,
        description="Nome de como conheceu a AIESEC",
        json_schema_extra={"example": "atados"},
    )

    @model_validator(mode="after")
    def verificar_origem_no_cache(self) -> Self | None:
        """Validador de negócio executado após a higienização que audita a origem em relação ao cache.
        ...
        Returns:
            Self | None: Retorna a própria instância validada do modelo Origem.

        Raises:
            ValueError: Caso haja divergência cadastral entre o ID da origem e o nome informado.
        """
        # Encaminha as chaves consolidadas do modelo para auditoria contra os metadados ativos
        valido = validar_dados_origem(nome=self.nome, id=self.id)

        if valido is True:
            return self

        # Comparação de erro idêntica caractere por caractere para levantar exceções de validação apropriadas
        if valido == "Não possui o external_id tag-origem-2 no cache":
            raise ValueError("Dados Inválidos: Não foi achada a referência de origem local")

        elif valido == "ID da origem informado não foi encontrado nas opções":
            raise ValueError("Dados Inválidos: O ID da origem informado não está presente nos dados")

        elif valido == "O nome da origem não corresponde ao ID informado":
            raise ValueError("Dados Inválidos: O nome da origem está incoerente com o ID informado")
